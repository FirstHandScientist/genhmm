...
class Glow(nn.Module):
    ...
    def forward(self, x_=None, y_onehot=None, z=None,
                eps_std=None, reverse=False):
        x = torch.FloatTensor(
            np.full((1, 3, 2 * x_.shape[0], 2 * x_.shape[0]), np.repeat(x_, 2, axis=0)))
...
class LatMM(nn.Module):
    ...
    def regulation_prior(self):
        ...
        for the_source in self.source_list:
            log_regulation.append(
                modules.Gamma.logp(alpha=alpha, beta=beta, x=torch.exp(the_source.logs), device=self.data_device))
    ...
    def forward(self, x=None, y_onehot=None, z=None, eps_std=None, reverse=False, idx=None, regulate_std=True):
        ...
        if not reverse:
            ...
            # if code_ is None, sample from Gaussian
            if z is None:
                B, C, H, W = self.graph.prior_h.size()
                C = int(C / 2)  ## sample  a batch of code_
