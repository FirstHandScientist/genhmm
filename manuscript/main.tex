...
\item $\bm{q} = \left[ q_1, q_2, \cdots, q_{|\Ss|}\right]^\intercal$ is the initial distribution of $\bm{H}$ with $|\Ss|$ as cardinality of $\Ss$. For a state $s$, $q_s = p(s;\bm{H})$. In the following, we use $s_t$ to denote the state $s$ at time $t$.
...
Using HMM for signal representation is illustrated in Figure~\ref{fig:hmm}. The model assumption is that different instant signal of $\ubar{\bm{x}}$ is generated by a different signal source associated with a hidden state of HMM, while the source selection is done by a discrete hidden markov process.
...
The probabilistic model of GenHMM for each hidden state is a mixture of $K$ neural network based generators, where $K$ standards for positive integer.
...
\begin{proof}
  ...
  \end{align*}
  Due to the fact that $KL(p(\ubm{s}, \ubm{\kappa}|\ubm{x};
...
\begin{table}
  ...
  \vspace{-0.3cm}
  \centering
  \subcaption{Folded $39$ Phonemes}\label{tab:acc-classification13f_a}
  \vspace{-0.1cm}
  \centering  
  \begin{tabular}{l|c|c|c}
    \toprule
  ...
  \end{tabular}
  \vspace{0.1cm}
  \subcaption{$61$ Phonemes}\label{tab:acc-classification13f_b}
  \vspace{-0.1cm}
  \begin{tabular}{l|c|c|c}
    \toprule
    {Model} &  K=1 &  K=3 &  K=5
