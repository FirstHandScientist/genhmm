...
HMM provides an framework for sequential data modeling. To connect states of HMM with input data, a probabilistic model is used to describe how each state of HMM should represent a pattern of sequential data. The typical way is to use a Gaussian mixture model (GMM) per state of HMM \cite{juang1986maximum}, where GMMs are used to represent the relationship between states of HMM and sequential data input. GMM based HMM (GMM-HMM) has becomes a standard model for sequential data modeling, and employed widely for practical applications, especially in speech recognition \cite{gales2008application,chatterjee2011auditory}.
...
Another track of related work is hybrid method of temporal neural network models and HMM. In \cite{liu2019lstmHmmHyb,buys2018bridging,vik2016rnnHmm}, a long short-term memory (LSTM) model/recurrent neural network (RNN) is combined with HMM as hybridization. A hierarchical training is carried out by: i) training a HMM first, ii) and then doing modified training of LSTM using trained HMM. This training procedure is argued by the authors to use LSTM or RNN to fill in the gap where HMM can not learn.

% work of epfl fiting GMM with neural network...

The above work help improve modeling capacity of HMM based models by bringing in neural networks. A softmax layer is usually used to represent probability whenever a conditional distribution is needed. These hierarchical schemes are built based on intuition of domain knowledge. Training of these hierarchical models usually need specific experience in relevant area to be able to proceed with the hierarchical procedure of training. 

In this work, we propose a generator-mixed HMM model, termed as GenHMM, where generators are realized by neural networks to help the model gain high modeling capacity.
Our proposed model, GenHMM:
\begin{itemize}
  \item firstly has high modeling capacity of sequential data, due to the neural network based generators;
\item has advantage of easy to do training, since GenHMM training employs expectation maximization (EM) framework. Training a GenHMM is as easy as training a GMM-HMM model, while configuration of GenHMM is flexible.
...
\end{itemize}
Instead of using softmax for probability representation, our GenHMM is able to compute exact loglikelihood of given sequence data based on change of variable formula. To make the loglikelihood computation efficient, neural network based generators of GenHMM are implemented as flow models. 

Our contributions in the paper are as follows:
\begin{itemize}
\item Proposing a neural network based HMM for sequential data modeling, i.e. GenHMM. The likelihood computation of sequential data under GenHMM can be computed exactly and efficiently.
...
\item Convergence analysis for GenHMM under the proposed learning algorithm.
\end{itemize}
...
\end{equation}
where $\kappa$ is a random variable following a categorical distribution, and $\pi_{s, \kappa} = p(\kappa | s; \bm{H})$. Naturally $\sum_{\kappa = 1}^{K} \pi_{s, \kappa}= 1$. Denote $\bm{\pi}_{s} = [\pi_{s,1}, \pi_{s,2}, \cdots, \pi_{s,K}]$,
$\bm{\theta}_s = \left\{ \bm{\theta}_{s, \kappa}| \kappa = 1, 2, \cdots, K \right\}$. 
We define $p(\bm{x}| s, \kappa; \bm{\theta}_{s, \kappa})$ as induced distribution by a generator $\bm{g}_{s,\kappa}: \RR^{N}\rightarrow\RR^{N}$, such that $\bm{x}=\bm{g}_{s, \kappa}(\bm{z})$, where $\bm{z}$ is a latent variable following a distribution with density function $p_{s,\kappa}(\bm{z})$. By change of variable, we have
...
The first problem that we need to address is to deal with the hidden sequential variable $\ubm{s}$ and $\ubm{\kappa}$  in model $\bm{H}$. For a sequential observable variable $\ubm{x}$, $\ubm{s}$ is the hidden state sequence corresponding to $\ubm{x}$, and $\ubm{\kappa}$ is the hidden variable sequence representing the generator identity sequence that actually generates $\ubm{x}$.

Since directly maximizing likelihood is not an option for our problem in \ref{eq:ml-of-hmm}, we address this problem in expectation maximization (EM) framework. This divides our problem into two steps: i) using the joint posterior of hidden variable sequences $\ubm{s}$ and $\ubm{\kappa}$ to obtain a ``expected likelihood'' of observable variable sequence $\ubm{x}$, i.e. the E-step; ii) maximizing the expected likelihood with regarding to (w.r.t.) the model $\bm{H}$, i.e. the M-step. We formulate these two steps as follows:
\begin{itemize}
...
We can see that the solution of $\bm{H}$ depends on the posterior probability $p(\ubm{s}| \ubm{x}; \bm{H})$. Though the evaluation of posterior according to Bayesian theorem is straightforward, the computation complexity of $p(\ubm{s}| \ubm{x}; \bm{H})$ grows exponentially with the length of $\ubm{s}$. Therefore, we employ forward-backward algorithm \cite{Bishop:2006:PRM:1162264} to do the posterior computation efficiently. As we would detail in next section, it is actually $p(s| \ubm{x}; \bm{H})$ and $p(s, \kappa| \ubm{x}; \bm{H})$ that are needed to formulate the problem. For the joint posterior $p(s, \kappa| \ubm{x}; \bm{H})$, it can be computed by following the Bayesian rule when posterior of hidden state is available.


With such a solution framework ready for model GenHMM, there are still remaining problems to address before it can be employed for practical usage, including:
\begin{itemize}
...
The convergence analysis is discussed at the end of this section.
...
Define  $\bm{g}_{s,\kappa}$ as a $L$-layer neural network and we formulate its mapping by each layer's concatenation:
$\bm{g}_{s,\kappa}=\bm{g}_{s,\kappa}^{[L]}\circ \bm{g}_{s,\kappa}^{[L-1]}\circ \cdots
\circ \bm{g}_{s,\kappa}^{[1]}$, where superscipt $[l]$ denotes the $l$'s layer index and $\circ$ denotes mapping concatenation. Assume $\bm{g}_{s,\kappa}$ is invertible and denotes its inverse mapping as $\bm{f}_{s,\kappa}=\bm{g}_{s,\kappa}^{-1}$. For a latent variable $\bm{z}$ with density function $p_{s,\kappa}(\bm{z})$, the generated signal $\bm{x}$ follows an induced distribution with density function \eqref{eq:changel-variable}. We illustrate the signal flow between latent variable $\bm{z}$ and observable variable $\bm{x}$ as follows:
\begin{equation*}
  \vspace{-8pt}
  \centering
  \begin{tikzpicture}
...
the full feed-forward neural network is invertible. Flow model, proposed in \cite{DBLP:journals/corr/DinhKB14} as an image generating model, is such a invertible
...
We firstly address the generator learning problem, i.e. $\umax{\bm{\Theta}} \Qq(\bm{\Theta}; \bm{H}^{\mathrm{old}})$. This is boiled down to maximizing the cost function of neural networks that can be formulated as
...
\end{align}
In \eqref{eq:obj-q-gen-mix}, the state posterior $p(s_t| \ubm{x}, \bm{H}^{\mathrm{old}})$ is computed by forward-backward algorithm. The posterior of $\kappa$ is:
...
\end{align}
where the last equation is due to the fact that $\bm{x}_t$ among sequence $\ubm{x}$ depends on $s_t, \kappa_t$ only. 

By substituting \eqref{eq:changel-variable} and \eqref{eq:cat-jacobian} into \eqref{eq:obj-q-gen-mix}, we have cost function for neural networks as
\begin{align}\label{eq:obj-q-gen-mix}
  &\Qq(\bm{\Theta}; \bm{H}^{\mathrm{old}}) \nonumber \\
...
With the generative modeling learning obtained, it remains to solve the initial distribution update and transition matrix update of HMM in GenHMM, i.e. the problem \eqref{eq:init-distribution-update} and \eqref{eq:transition-update}. These two problem are basically two constrained optimization problems. The solution to them is provided in \cite{Bishop:2006:PRM:1162264}. But to keep learning algorithm for GenHMM complete, we give the update rules for $\bm{q}$ and $\bm{A}$ as follows.

\subsubsection{Initial Probability Update}
Problem in \eqref{eq:init-distribution-update} can be reformulated as:
\begin{align}
  \Qq(\bm{q}; \bm{H}^{\mathrm{old}})
...
$p(\smtr{s}{1}{r};\bm{H})$ is the probability of initial state of GenHMM for $r$-th sequential sample. Actually $q_i = p({s}_{1} =i) $, $i= 1, 2, \cdots, |\Ss|$ for $\bm{H}$. Solution to problem:
\begin{align}
...
Problem \eqref{eq:transition-update} can be reformulated as
\begin{align}
...
Since $\bm{A}_{i, j}  = p(\smtr{s}{t+1}{r}=j|\smtr{s}{t}{r}=i; \bm{H})$ is the element of transition matrix $\bm{A}$, the solution to problem:
...
\subsection{Algorithm of GenHMM and Convergence Analysis}

To summarize the learning solution in subsection~\ref{subsec:optmGenHMM}, we wrap our algorithm into pseudocode as shown in Algorithm~\ref{algo:genhmm}. We use Adam \cite{DBLP:journals/corr/KingmaB14} optimizer for optimization w.r.t. the parameters of generators in GenHMM. As is shown from line $6$ to $12$ in Algorithm~\ref{algo:genhmm}, the batch-size stochastic gradient decent can be naturally embedded into the learning algorithm of GenHMM.

As described by the pseudocode in Algorithm~\ref{algo:genhmm}, the learning of GenHMM is divided into optimization w.r.t. to generators' parameters $\bm{\Theta}$, initial probability $\bm{q}$ of hidden state, transition matrix $\bm{A}$, and generator mixture weights $\bm{\Pi}$. Different from the optimization w.r.t. to $\bm{q}$, $\bm{A}$ and $\bm{\Pi}$, which have optimal solutions, generator learning usually cannot give optimal solution to problem $\max_{\bm{\Theta}} \Qq(\bm{\Theta}; \bm{H}^{\mathrm{old}})$. In fact, given that no optimal $\bm{\Theta}$ is obtained in each iteration in Algorithm~\ref{algo:genhmm}, learning of GenHMM can still converge.

In pursuit of representing a dataset by GenHMM, we are actually minimize a KL divergence $KL(\hat{p}(\ubm{x})\| p(\ubm{x};\bm{H}))$. Our following analysis shows that our proposed algorithm for learning GenHMM can converge, without requirement for optimal $\bm{\Theta}$ in each iteration.
...
\begin{prop}
  Assume that parameters of GenHMM are in a compact set. Further assume that $\Qq(\bm{\Theta}^{\mathrm{new}}; \bm{H}^{\mathrm{old}}) \geq \Qq(\bm{\Theta}^{\mathrm{old}}; \bm{H}^{\mathrm{old}})$, then we always have $KL(\hat{p}(\ubm{x})\| p(\ubm{x};\bm{H}^{\mathrm{new}})) \leq KL(\hat{p}(\ubm{x})\| p(\ubm{x};\bm{H}^{\mathrm{old}}))$, and GenHMM converges.
\end{prop}
...
\begin{table}
  ...
  \begin{tabular}{l|c|c|c}
    ...
    GMM-HMM & $62.30\%$ &  $68.06\%$ &  $68.73\%$  \\ \midrule
    GenHMM & $76.78\%$ & $77.77\%$ &  $\%$ \\ 
    \bottomrule                                                                  
  \end{tabular}
  \bigskip
  ...
  \begin{tabular}{l|c|c|c} \toprule
    {Model} &  K=1 &  K=3 &  K=5
    \\ \midrule
    GMM-HMM & $53.64\%$ &  $59.65\%$ & $61.98\%$  \\ \midrule
    GenHMM & $69.51\%$ & $70.67\%$ & $70.78\%$   \\ 
    \bottomrule
  \end{tabular}
  \vspace{0.3cm}
\end{table}
...
\begin{table}
  ...
  \begin{tabular}{l|c|c|c}
    \toprule
    {Model} &  K=1 &  K=3 &  K=5  \\  \midrule
    GMM-HMM & $48.59\%$ &  $51.26\%$ &  $52.40\%$  \\ \midrule
    GenHMM & $61.10\%$ & $\%$ &   \\ 
    \bottomrule                                                                  
  \end{tabular}
  \bigskip
  ...
  \begin{tabular}{l|c|c|c}
    ...
    {Model} &  K=1 &  K=3 &  K=5
    \\ \midrule
    GMM-HMM & $37.15\%$ &  $40.65\%$ & $42.27\%$  \\ \midrule
    GenHMM & $50.33\%$ & $\%$ & $\%$   \\ 
    \bottomrule
  \end{tabular}
  \vspace{0.3cm}
...
Similar experiments to that in table~\ref{tab:acc-classification13f} are carried out by using only the MFCC coefficients as feature input (excluding deltas and delta-deltas). The results are shown in table~\ref{tab:acc-classification13f}. The superior performance of GenHMM remains compared with reference model GMM-HMM. The gain by using mixture generators are also present in this set of experiments while the difference between $61$-phoneme and $39$-phoneme cases is also similar to the set of experiments in table~\ref{tab:acc-classification39f}.

\section{Conclusion}
In this work, we proposed a generative model based HMM (GenHMM) whose generators are realized by neural networks. We provide the training method for GenHMM. The validity of GenHMM is demonstrated by the experiments of classification task on practical sequential dataset. The learning method in this work is based on generative training. Future work would consider discriminative training for classification tasks of sequential data.
