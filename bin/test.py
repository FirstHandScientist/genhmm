...
if __name__ == "__main__":
    ...
    n_components = 3
    n_prob_components = 2
    hmm1 = GenHMM(n_components=n_components, n_prob_components=n_prob_components,
                  n_iter=3000,
                    em_skip=100, tol=0)
    ...
    data1_n = (data1 - np.min(data1, axis=1).reshape(-1, 1)) / (
                np.max(data1, axis=1).reshape(-1, 1) - np.min(data1, axis=1).reshape(-1, 1))

    limit = 10
    hmm1.fit(data1_n[:np.cumsum(length1[:limit])[-1]], length1[:limit])
    print(len(length1))
